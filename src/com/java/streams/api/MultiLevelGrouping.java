...
import com.java.streams.model.Book.Type;
...
public class MultiLevelGrouping {
	...
	private static Map<PriceLevel, List<Book>> groupBooksByPriceLevel() {
		return Book.menu.stream().collect(groupingBy(book -> {
			if (book.getPrice() <= 400)
				return PriceLevel.CHEAP;
			else if (book.getPrice() <= 700)
				return PriceLevel.NORMAL;
			else
				return PriceLevel.COSTLY;
		}));
	}

	private static Map<Book.Type, Map<PriceLevel, List<Book>>> groupBooksByTypeAndPriceLevel() {
		return Book.menu.stream().collect(groupingBy(Book::getType, groupingBy((Book book) -> {
			if (book.getPrice() <= 400)
				return PriceLevel.CHEAP;
			else if (book.getPrice() <= 700)
				return PriceLevel.NORMAL;
	...
	private static Map<Type, Object> costliestBooksByTypeWithoutOptionals() {
		return Book.menu.stream().collect(groupingBy(Book::getType,
				collectingAndThen(reducing((b1, b2) -> b1.getPrice() > b2.getPrice() ? b1 : b2), Optional::get)));
	}
